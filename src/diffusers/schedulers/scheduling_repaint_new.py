...
class RePaintSchedulerNew(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        return RePaintSchedulerOutput(prev_sample=pred_prev_sample, pred_original_sample=prev_unknown_part)
    
    def initial_step(self,noise,hair_image,timestep):
        alpha_prod_t = self.alphas_cumprod[timestep]
        #noise = randn_tensor(model_output.shape, generator=generator, device=device, dtype=model_output.dtype)
        prev_known_part = (alpha_prod_t**0.5) * hair_image + ((1 - alpha_prod_t) ** 0.5) * noise
        return prev_known_part
